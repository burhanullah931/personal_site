...
<div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
      <form id="RegisterValidation" action="{{action('Admin\MaintenanceController@update', $maintenance->id)}}" method="POST" novalidate="novalidate" enctype="multipart/form-data">
             @csrf
             @method('PUT')
                <div class="card ">
                  ...
                  <input type="hidden" name="maintenance_id" value="{{$maintenance->id}}" >
                      <div class="form-group bmd-form-group ">
                        <label for="message" class="bmd-label-floating @error('message') has-danger @enderror"> Maintenance message</label>
                        <textarea rows="4" cols="50" class="form-control" name="message" id="message"  aria-required="true">{{$maintenance->message}}</textarea>
                        @error('message') <label id="message-error" class="error" for="message">{{$message}}</label>@enderror
                      </div>
